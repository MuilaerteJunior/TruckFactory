    ...
    public class InMemoryTests : BaseTests
    {
        ...
        public void Validate_Create()
        {
            ...
            using (var context = new FactoryContext(_contextOptions))
            {
                ...
                Assert.Equal(2021, entityCreated.ProductionYear);
        ...
        public void Validate_Rules()
        {
            ...
            using (var context = new FactoryContext(_contextOptions))
            {
                ...
                Assert.True(item.ContainsKey("ModelYear") && item["ModelYear"].Errors.Any());
                Assert.True(item.ContainsKey("ProductionYear") && item["ProductionYear"].Errors.Any());
            }
        }
    }
}